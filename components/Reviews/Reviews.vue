    ...
    <div class="container">
      ...
      >
        <slide class="testimonial" v-for="(testimonial, index) in testimonials" :key="index">
          ...
          <p class="testimonial-content">{{testimonial.comment | reduceText}}</p>
...
export default {
  ...
  async created() {
    try {
      const response = await axios.get(`${this.$API}/rate`);
      const instructorsData = await axios.get(`${this.$API}/instructors`);
